...
def main():
    ...
    try:
        ...
        timeout = -1
    try:
        libHit_confidence= params_obj["libHit_confidence"][0]
    except:
        libHit_confidence = 1
    try:
        cosine_threshold= params_obj["cosine_threshold"][0]
    except:
        cosine_threshold = 0
    ...
    if useSirius:
        ...
        if annot and not empty_exp:
            cmd = "%s --zodiac  --sirius %s --spectral-hits %s --thresholdfilter %s --minLocalConnections %s --output %s --processors %s --lowest-cosine %s --lambda %s --spectra %s"%(p_sirius,p_out_sirius,p_csv,filter,connections,p_out_zodiac,processor,cosine_threshold,libHit_confidence,lp_mgf)
            execute_script_file.write(cmd + "\n")
            fLog.write(cmd)
            fLog.write('\n')
        else:
            cmd = "%s --zodiac --sirius %s --thresholdfilter %s --minLocalConnections %s --output %s --processors %s --lowest-cosine %s --lambda %s --spectra %s"%(p_sirius,p_out_sirius,filter,connections,p_out_zodiac,processor,cosine_threshold,libHit_confidence,p_mgf)
            execute_script_file.write(cmd + "\n")
            fLog.write(cmd)
            fLog.write('\n')
        #step3:
        cmd = "%s --fingerid --fingerid-db %s --experimental-canopus=%s --ppm-max %s -o %s %s" %(p_sirius,FID_DB,p_c,ppm,p_out_fingerid,p_out_zodiac)
        execute_script_file.write(cmd + "\n")
        fLog.write(cmd)
        fLog.write('\n')
    #execution
